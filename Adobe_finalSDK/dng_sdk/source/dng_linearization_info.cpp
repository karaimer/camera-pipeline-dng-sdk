...
void dng_linearize_plane::Process (const dng_rect &srcTile)
	...
	//std::string save_directory = ".\\stage3_image"; // read from image folder in each case. 
	...
	if(last_stage == 3 || last_stage == 2 ){
		// hakki can karaimer commented out edited
		// This part reads Stage3 image from file. 
		// Do not forget to off the gainmap setting on Matlab.
		if(rw_settings == 1){
			//for(int tPtrRindex = 0; tPtrRindex<srcCols;++tPtrRindex){
			//	fs1 >> tPtrR[tPtrRindex];																					
			//}//end for 
			//printf("reading stage %d, ", last_stage ," image\n" );
